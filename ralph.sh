...
run_project() {
  ...
  # Headless auth: export OAuth token if available (same pattern as ted-morning-brief/run.sh).
  # Without this, an unattended `claude -p` under launchd reads the ambient Keychain login,
  # which expires silently and 401s every iteration (ralph dark 2026-06-23→07-02).
  local _oauth_file="$HOME/.ted-secrets/claude-oauth.token"
  if [ -s "$_oauth_file" ]; then
    CLAUDE_CODE_OAUTH_TOKEN="$(tr -d '[:space:]' < "$_oauth_file")"
  ...
  # headless run. Guardrail is the global deny wall (permissions.deny in
...
Before concluding code does not exist, grep for it. No placeholder implementations."
  ...
  for i in $(seq 1 "$MAX_ITERS"); do
    ...
    if [ "$rc" -ne 0 ]; then
      # Persist WHY into the report — auth failures (API Error: 401) are otherwise
      # indistinguishable from code failures (10 dark nights, 2026-06-23→07-02).
      local _why=""
      [ -n "$ITER_ERR" ] && [ -s "$ITER_ERR" ] && _why="$(tail -n 3 "$ITER_ERR" | tr '\n' ' ')"
      if [ -z "$_why" ] && [ -n "$ITER_LOG" ] && [ -s "$ITER_LOG" ]; then
        _why="$(grep -im1 'error' "$ITER_LOG" 2>/dev/null || tail -n 1 "$ITER_LOG" 2>/dev/null || true)"
      fi
      _why="${_why:0:400}"
    ...
    # Iteration captures are consumed (cost + failure reason) — clean up after failure branch.
